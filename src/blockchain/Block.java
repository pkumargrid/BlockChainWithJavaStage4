...
import magic.generator.MagicGenerator;
...
public class Block {
    ...
    public synchronized void setStatus(int currentTime) {
        BlockChain blockChain = BlockChain.getInstance(constants.Block.CAPACITY.value);
        if(currentTime > 6){
    ...
    public synchronized void createHash(){
        BlockChain blockChain = BlockChain.getInstance(constants.Block.CAPACITY.value);
        MagicGenerator.HashMagic hashMagic =  MagicGenerator.generate(  id + prevHash + timeStamp + getMessages(), blockChain.getNumberOfZeros(), 0);
